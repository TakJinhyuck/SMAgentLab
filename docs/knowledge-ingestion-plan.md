...
|----------|-----------|---------------|------|
| operation_manual | hierarchical_sections | `section` | ## 헤더 기준 분할 |
| troubleshooting_guide | hierarchical_sections | `section` | 문제-원인-해결 단위 |
| api_doc | hierarchical_sections | `section` | 엔드포인트/섹션 단위 |
| tabular_data | table_rows | `paragraph` | 단락 기반 + 테이블 청크 |
| meeting_notes | flat_paragraphs | `paragraph` | 단락(빈 줄) 기반 분할 |
| faq | qa_pairs | `paragraph` | Q-A 쌍이 빈 줄로 구분 |
| mixed | mixed | `auto` | chunker가 자동 판단 |
...
class StrategyPlanner:
    def execute(self, analysis: dict, source: dict) -> list[ChunkPlan]:
        ...
        if strategy == "section":
            # 마크다운 헤더(##, ###) 기준 분할 → 작은 섹션 병합, 큰 섹션 재분할
            chunks = chunker.chunk_document(doc, strategy="section")

        elif strategy == "paragraph":
            # 빈 줄 기준 분할 → 작은 단락 병합, 큰 단락 재분할
            chunks = chunker.chunk_document(doc, strategy="paragraph")
...
### Tier 3: "파일 던지면 끝" 완성 (목표: 3~5일)

Analyzer Agent + 자동 Q&A 생성으로 사용자 개입을 최소화.

| # | 기능 | 변경 내용 | 파일 |
|---|------|----------|------|
| **3-1** | Analyzer Agent | LLM이 문서 유형/구조 분석 → 최적 청킹 전략 자동 결정. Tier 2에서 사용자가 전략을 수동 선택하던 부분을 AI가 대체 | 신규: `ingestion/analyzer.py` |
| **3-3** | 자동 Q&A 생성 | 등록된 지식 청크에서 LLM이 예상 질문-답변 쌍 생성 → `rag_fewshot`에 candidate로 등록. Few-shot 수동 작성 불필요 | 신규: `ingestion/qa_gen.py` |
...
Week 1 ─── Tier 1 ──────────────────────────────── ✅ DONE
  ...
  │
Week 1 ─── Tier 2 ──────────────────────────────── ✅ DONE
  │  파일 업로드 + 섹션 기반 청킹 + LLM 태깅 + 용어 추출
  │  → 핵심 UX: "PDF 던지면 자동으로 지식화 + 메타데이터"
  │
Week 2 ─── Tier 3 (3-1 + 3-3) ─────────────────── ✅ DONE
     Analyzer Agent + 자동 Q&A 생성
     → "파일 던지면 끝" 완성
```
...
| 100페이지 문서 등록 | 2~3일 (안 함) | 30분 (CSV) | 5분 (업로드) | 2분 (전략도 자동) |
| 메타데이터 품질 | 사람마다 다름 | 수동 | LLM 일관 태깅 | LLM 분석 + 태깅 |
| 용어집 관리 | 별도 수동 | 수동 | 자동 추출 | 자동 추출 |
| Few-shot 관리 | 수동 등록 | 수동 | 수동 | 자동 Q&A 생성 |
...
|----|------|------|----------|------|
| **2-1** | **파일 업로드 + 섹션 기반 청킹** | | | |
| 2-1-1 | 파일 파싱 어댑터 (TXT/MD → 섹션 분할) | `DONE` | `ingestion/adapters.py` | parse_text, parse_markdown, parse_file |
| 2-1-2 | PDF 파싱 어댑터 (pymupdf) | `DONE` | `ingestion/adapters.py` | pymupdf/fitz fallback |
| 2-1-3 | 파일 업로드 API endpoint (multipart) | `DONE` | `knowledge/router.py` | POST /import/file + /import/file/preview |
| 2-1-4 | 청킹 엔진 (섹션/단락/고정크기 전략) | `DONE` | `ingestion/chunker.py` | section/paragraph/fixed + 테이블 청크 |
| 2-1-5 | 프론트: 파일 업로드 모달 (드래그&드롭, 옵션, 미리보기) | `DONE` | `KnowledgeTable.tsx` | FileUploadModal + LLM 옵션 체크박스 |
| **2-2** | **LLM 자동 메타데이터 태깅** | | | |
| 2-2-1 | 메타데이터 추출 LLM 프롬프트 설계 | `DONE` | `ingestion/tagger.py` | _TAGGER_SYSTEM/PROMPT |
| 2-2-2 | 청크별 category, container_name 자동 태깅 | `DONE` | `ingestion/tagger.py` | auto_tag_chunks() + LLM fallback |
| 2-2-3 | base_weight 자동 추정 (priority_score → weight) | `DONE` | `knowledge/router.py` | 0.5 + score * 1.5 |
| **2-3** | **용어집 자동 추출** | | | |
| 2-3-1 | 인제스천 시 도메인 용어 자동 감지 | `DONE` | `ingestion/tagger.py` | extract_glossary_terms() |
| 2-3-2 | 기존 용어 중복 체크 (임베딩 유사도 > 0.9) | `DONE` | `ingestion/tagger.py` | existing_terms 필터링 |
| 2-3-3 | pending_review 상태로 rag_glossary 등록 | `DONE` | `knowledge/router.py` | import_file 내 auto_glossary 옵션 |
| **2-4** | **인제스천 진행 상태 UI** | | | |
| 2-4-1 | 비동기 인제스천 처리 (asyncio.create_task) | `DEFER` | - | 현재 동기 처리로 충분, 대용량 파일 시 추후 적용 |
| 2-4-2 | 진행률 SSE 이벤트 또는 폴링 API | `DEFER` | - | job 테이블로 폴링 가능, SSE는 추후 |
| 2-4-3 | 프론트: 실시간 진행률 바 + 완료 알림 | `DONE` | `KnowledgeTable.tsx` | 등록 완료 후 alert + 이력 패널 자동 갱신 |

### Tier 3: "파일 던지면 끝" 완성 (목표: 3~5일)

| ID | 작업 | 상태 | 변경 파일 | 비고 |
|----|------|------|----------|------|
| **3-1** | **Analyzer Agent** | | | LLM이 문서 분석 → 청킹 전략 자동 결정 |
| 3-1-1 | 문서 분석 LLM 프롬프트 (유형/구조/도메인/중요도/청킹전략) | `DONE` | `ingestion/analyzer.py` | _ANALYZER_SYSTEM/PROMPT |
| 3-1-2 | 분석 결과 → chunk_document() 전략 자동 매핑 | `DONE` | `ingestion/analyzer.py` | _STRATEGY_MAP + _validate_and_normalize |
| 3-1-3 | /import/file endpoint에 auto_analyze 옵션 추가 | `DONE` | `knowledge/router.py` | 전략 오버라이드 + 카테고리 자동 |
| 3-1-4 | 분석 결과를 rag_ingestion_job.analyzer_result에 저장 | `DONE` | `knowledge/router.py` | JSONB 저장 |
| 3-1-5 | 프론트: 분석 결과 미리보기 (문서유형/전략) + AI 분석 체크박스 | `DONE` | `KnowledgeTable.tsx` | autoAnalyze → 전략 비활성화 |
| **3-3** | **자동 Q&A 생성** | | | 지식 청크 → LLM Q&A → fewshot candidate |
| 3-3-1 | Q&A 생성 LLM 프롬프트 설계 | `DONE` | `ingestion/qa_gen.py` | generate_qa_pairs + bulk_generate_qa |
| 3-3-2 | 생성된 Q&A를 rag_fewshot에 candidate로 등록 | `DONE` | `knowledge/router.py` | embedding + INSERT |
| 3-3-3 | /import/file endpoint에 auto_fewshot 옵션 추가 | `DONE` | `knowledge/router.py` | 상위 5청크 제한 (비용) |
| 3-3-4 | 프론트: auto_fewshot 체크박스 + 결과 알림 | `DONE` | `KnowledgeTable.tsx` | emerald 색상 체크박스 |
| 3-3-5 | 단위 테스트 (analyzer + qa_gen) | `DONE` | `tests/test_ingestion_tier3.py` | 23 TC 전체 통과 |
...
| 2026-04-15 | v1.3 | 1-1-9 | 자체 테스트 32 TC 작성 + BOM 처리 버그 수정 + tsc 통과 확인 | `7c12cae` |
| 2026-04-15 | v1.4 | 2-1 ~ 2-4 | Tier 2 구현 (파일 업로드+청킹+LLM태깅+용어추출+UI) + 34 TC | `5c7ea1d` |
| 2026-04-15 | v1.5 | - | Tier 3 범위 조정: 3-1(Analyzer)+3-3(Q&A)만 유지, 3-2/3-4/3-5 제외 | `e5453d6` |
| 2026-04-15 | v1.6 | 3-1 + 3-3 | Tier 3 전체 구현 + 23 TC 통과 (총 89 TC) | - |
