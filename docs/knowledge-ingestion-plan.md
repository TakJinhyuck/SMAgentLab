...
|----|------|------|----------|------|
| **1-1** | **CSV/JSON 벌크 임포트** | | | |
| 1-1-1 | DB 마이그레이션: `source_file`, `source_chunk_idx`, `source_type` 컬럼 추가 | `TODO` | `main.py` | |
| 1-1-2 | DB 마이그레이션: `rag_ingestion_job` 테이블 생성 | `TODO` | `main.py` | |
| 1-1-3 | Pydantic 스키마에 source 필드 추가 | `TODO` | `knowledge/schemas.py` | |
| 1-1-4 | 벌크 등록 service 함수 (`bulk_create_knowledge`) | `TODO` | `knowledge/service.py` | embed_batch 사용 |
| 1-1-5 | CSV 파싱 + 벌크 등록 API endpoint | `TODO` | `knowledge/router.py` | multipart/form-data |
| 1-1-6 | 인제스천 작업 CRUD service | `TODO` | `knowledge/service.py` | job 생성/조회/업데이트 |
| 1-1-7 | 프론트: CSV 임포트 모달 (파일선택 + 컬럼매핑 + 미리보기) | `TODO` | `KnowledgeTable.tsx` | |
| 1-1-8 | 프론트: API 클라이언트 함수 추가 | `TODO` | `api/knowledge.ts` 또는 기존 | |
| 1-1-9 | 단위 테스트 (벌크 등록, CSV 파싱) | `TODO` | `tests/` | |
| **1-2** | **대량 텍스트 붙여넣기 → 자동 분할** | | | |
| 1-2-1 | 텍스트 분할 유틸 함수 (`split_text_to_chunks`) | `TODO` | `knowledge/service.py` | `\n\n`, `---`, `##` 기준 |
| 1-2-2 | 프론트: 텍스트 붙여넣기 모달 (분할 기준 선택 + 미리보기) | `TODO` | `KnowledgeTable.tsx` | |
| 1-2-3 | 분할 결과 → bulk_create_knowledge 호출 | `TODO` | `knowledge/router.py` | |
| **1-3** | **지식 리스트 소스 표시** | | | |
| 1-3-1 | 기존 리스트 API 응답에 source 필드 포함 | `TODO` | `knowledge/service.py` | |
| 1-3-2 | 프론트: 소스 뱃지 + 소스 필터 드롭다운 | `TODO` | `KnowledgeTable.tsx` | |
| **1-4** | **인제스천 작업 이력 UI** | | | |
| 1-4-1 | 인제스천 작업 목록 API | `TODO` | `knowledge/router.py` | |
| 1-4-2 | 프론트: 작업 이력 패널 (상태, 청크 수, 소요시간) | `TODO` | `KnowledgeTable.tsx` | |
...
| 2026-04-15 | v1.1 | - | WBS 추가, dev_0 브랜치 작업 시작 | - |
