...
> **Version**: 1.5
> **작성일**: 2026-04-15
> **상태**: Tier 2 완료, Tier 3 (3-1 + 3-3) 구현 대기
> **브랜치**: `dev_0`
...
| 3-1-1 | 문서 분석 LLM 프롬프트 (유형/구조/도메인/중요도/청킹전략) | `TODO` | 신규: `ingestion/analyzer.py` | |
| 3-1-2 | 분석 결과 → chunk_document() 전략 자동 매핑 | `TODO` | `ingestion/analyzer.py` | doc_type → strategy 변환 |
| 3-1-3 | /import/file endpoint에 auto_analyze 옵션 추가 | `TODO` | `knowledge/router.py` | 기존 chunk_strategy='auto' 대체 |
| 3-1-4 | 분석 결과를 rag_ingestion_job.analyzer_result에 저장 | `TODO` | `knowledge/router.py` | JSONB 필드 활용 |
| 3-1-5 | 프론트: 분석 결과 미리보기 (문서유형/전략/예상 청크 수) | `TODO` | `KnowledgeTable.tsx` | FileUploadModal 확장 |
| **3-3** | **자동 Q&A 생성** | | | 지식 청크 → LLM Q&A → fewshot candidate |
| 3-3-1 | Q&A 생성 LLM 프롬프트 설계 | `TODO` | 신규: `ingestion/qa_gen.py` | 청크당 1~3쌍 생성 |
| 3-3-2 | 생성된 Q&A를 rag_fewshot에 candidate로 등록 | `TODO` | `ingestion/qa_gen.py` + `service.py` | knowledge_id 연결 |
| 3-3-3 | /import/file endpoint에 auto_fewshot 옵션 추가 | `TODO` | `knowledge/router.py` | |
| 3-3-4 | 프론트: auto_fewshot 체크박스 + 결과 알림 | `TODO` | `KnowledgeTable.tsx` | "Q&A N건 자동 생성" |
| 3-3-5 | 단위 테스트 (analyzer + qa_gen) | `TODO` | `tests/` | |
...
| 2026-04-15 | v1.5 | - | Tier 3 범위 조정: 3-1(Analyzer)+3-3(Q&A)만 유지, 3-2/3-4/3-5 제외 | - |
