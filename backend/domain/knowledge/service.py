    ...
    *,
    ...
    async with get_conn() as conn:
        current = await conn.fetchrow(
            "SELECT k.*, n.name AS ns_name FROM ops_knowledge k JOIN ops_namespace n ON k.namespace_id = n.id WHERE k.id = $1",
            knowledge_id,
        )
        if not current:
            return None
...
async def get_knowledge_part(knowledge_id: int) -> Optional[str]:
    ...
    async with get_conn() as conn:
        return await conn.fetchval(
            "SELECT created_by_part FROM ops_knowledge WHERE id = $1", knowledge_id
        )
...
async def get_knowledge_namespace(knowledge_id: int) -> Optional[str]:
    ...
    async with get_conn() as conn:
        return await conn.fetchval(
            "SELECT n.name FROM ops_knowledge k JOIN ops_namespace n ON k.namespace_id = n.id WHERE k.id = $1",
            knowledge_id,
        )
...
async def create_glossary(
    ...
    *, created_by_part: Optional[str] = None, created_by_user_id: Optional[int] = None,
    ...
    async with get_conn() as conn:
        ...
        row = await conn.fetchrow(
            """
            INSERT INTO ops_glossary (namespace_id, term, description, embedding, created_by_part, created_by_user_id)
    ...
    *, updated_by_part: Optional[str] = None, updated_by_user_id: Optional[int] = None,
    ...
    async with get_conn() as conn:
        ...
        ns_name = await conn.fetchval(
            "SELECT n.name FROM ops_glossary g JOIN ops_namespace n ON g.namespace_id = n.id WHERE g.id = $1",
            glossary_id,
        )
...
async def get_glossary_part(glossary_id: int) -> Optional[str]:
    async with get_conn() as conn:
        return await conn.fetchval(
            "SELECT created_by_part FROM ops_glossary WHERE id = $1", glossary_id
        )
