...
async def get_fewshots(namespace: str, _=Depends(require_admin)):
    ns_id = await _get_ns_id(namespace)
    async with get_conn() as conn:
        rows = await conn.fetch(
            "SELECT id, question, sql, category, hits, last_hit, created_at "
            "FROM sql_fewshot WHERE namespace_id = $1 ORDER BY hits DESC",
            ns_id,
        )
    return [dict(r) for r in rows]
...
async def create_fewshot(namespace: str, body: FewshotPayload, _=Depends(require_admin)):
    ...
    async with get_conn() as conn:
        row_id = await conn.fetchval("""
            ...
            VALUES ($1, $2, $3, $4, $5::vector) RETURNING id
        """, ns_id, body.question, body.sql, body.category, str(emb))
    return {"id": row_id, "ok": True}
